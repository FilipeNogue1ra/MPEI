...
\begin{titlepage}
    ...
    {\Large Nome do Aluno 1 - NMec \par Nome do Aluno 2 - NMec \par}
    
    \vfill
    
    {\large Professor das Aulas Teóricas: \par António Joaquim da Silva Teixeira (ajst@ua.pt) \par}
...
\begin{figure}[htbp]
  \centering
  \includegraphics[width=0.55\textwidth]{fig_bloom_fp_vs_k_new.png}
  \caption{Taxa de falsos positivos vs. número de funções de hash ($k$) para o filtro de Bloom.}
  \label{fig:bloom-fp-vs-k}
\end{figure}
\begin{figure}[htbp]
  \centering
  \includegraphics[width=0.55\textwidth]{fig_bloom_fp_vs_m_new.png}
  \caption{Taxa de falsos positivos vs. tamanho do filtro ($m$) para o filtro de Bloom.}
  \label{fig:bloom-fp-vs-m}
\end{figure}

\noindent\textbf{Conclusão} \\
Os testes demonstram que a taxa empírica converge de forma muito precisa para a taxa teórica de $3.06\%$. Isto prova que as funções de dispersão (hash) baseadas no algoritmo djb2 distribuem os dados uniformemente pelo vetor de bits. Para reduzir as colisões/falsos positivos em ambiente clínico real, o tamanho $m$ do filtro deve ser aumentado.
...
\begin{figure}[htbp]
  \centering
  \includegraphics[width=0.55\textwidth]{fig_minhash_error_vs_N_new.png}
  \caption{Erro de estimativa do MinHash em função do número de hashes $N$.}
  \label{fig:minhash-error-vs-N}
\end{figure}
\begin{figure}[htbp]
  \centering
  \includegraphics[width=0.55\textwidth]{fig_minhash_k_compare_new.png}
  \caption{Comparação de desempenho do MinHash para diferentes valores de $k$ (tamanho do shingle).}
